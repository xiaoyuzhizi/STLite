...
namespace sjtu {
	...
	> class linked_hashmap : public list<pair<const Key, T> > {
	public:
		size_t cap;
		double loadf;
		...
		typedef pair<const Key, T> value_type;//pair don't need head file
        ...
        using pr = pair<value_type, node*>;//我需要标志本身的位置便于创造对应的迭代器
        using piterator = typename list<pr>::iterator;
        list<pr>** c;
		...
		linked_hashmap(const linked_hashmap& other) {
            cap = other.cap; loadf = other.loadf;
            c = new list<pr>*[cap];
            for (int i = 0; i < cap; ++i)
                c[i] = new list<pr>(*(other.c[i]));
        ...
        inline void resize(bool op=1)
        {
            int siz = this->size();
            if (siz < cap * loadf  && siz>0.1*cap)
            {
                return;

            }
            if (siz <= 0.1 * cap && op)return;

            if (siz >= cap * loadf && cap == max_size)return;
            ...
            {
                pos = Hash()(i->data->first) % cap;
                c[pos]->push_back(pr(*(i->data), i));
            }
        }
        ...
        inline pr* findd(int k, const Key& key)const
        {
            for (piterator i = c[k]->begin(); i != c[k]->end(); ++i)
            {
                if (Equal()((*i).first.first, key))
                    return &(*i);
            }
            return nullptr;
        }
        ...
        pair<iterator, bool> insert(const value_type& value) {
            int pos = Hash()(value.first) % cap;
            pr* tmp = findd(pos, value.first);
            if (tmp)
            {
                return pair<iterator, bool>(iterator(tmp->second, this), false);

            }
            
            this->push_back(value);
            c[pos]->push_back(pr(value,this->tail->pre));
            resize();
            return pair<iterator, bool>(iterator(this->tail->pre, this), true);
		...
		linked_hashmap& operator=(const linked_hashmap& other) {
            ...
            for (const_iterator i = other.cbegin(); i != other.cend(); ++i) {
                this->push_back(*i);
                int t = Hash()((*i).first) % cap;
                c[t]->push_back(pr(*i, this->tail->pre));
            }
            return *this;
		...
		T& at(const Key& key) {
            ...
            pr* tmp = findd(pos, key);
            if (!tmp)throw index_out_of_bound();
            return tmp->second->data->second;
		...
		T& operator[](const Key& key) {
            int pos = Hash()(key) % cap;//T的值赋多少暂时存疑
            pr* tmp = findd(pos, key);
            if (tmp)return tmp->second->data->second;
            const value_type t(key, T());
            node* t1 = new node(t);
            this->inserttt(this->tail, t1);

            
            c[pos]->push_back(pr(t, this->tail->pre));
            resize();
            return t1->data->second;
  ...
  //      }
        ...
        iterator erase(iterator pos) {
            if (pos == this->end()||pos.belong!=this) { throw index_out_of_bound(); }
            int k = Hash()((*pos).first) % cap;
            for (piterator i = c[k]->begin(); i != c[k]->end(); ++i)
            {
                if (Equal()((*pos).first, (*i).first.first))
                {   
		...
		iterator find(const Key& key) {
            int pos = Hash()(key) % cap;
            pr* tmp = findd(pos, key);
            if (!tmp)return this->end();
            return iterator(tmp->second, this);
        
        }
		const_iterator find(const Key& key) const {
            int pos = Hash()(key) % cap;
            pr* tmp = findd(pos, key);
            if (!tmp)return this->cend();
            return const_iterator(tmp->second, this);
